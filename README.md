...
**数学建模智能助手** 是一个面向数学建模竞赛（CUMCM 国赛 / MCM-ICM 美赛）的 LLM Agent 系统。它通过 **建模手 → 编程手 → 论文手** 三角色协作，配合 **五道独立质量门禁**、**数值验证引擎**、**陷阱检测系统**、**多算法对比机制** 和 **国赛级质量自动检测**，将完整的数学建模流程自动化，目标达到国赛一等奖水平。

### 核心能力

| 能力 | 说明 |
|------|------|
| 🧠 **智能建模分析** | 自动读题、拆分子问题、选择模型、设计求解方案，要求每子问题 ≥ 1 个创新改进点，强制输出理论上界 |
| 💻 **自动代码生成与修复** | 生成可运行 Python 代码，内置 8 层修复管道确保语法正确，强制 ≥3 种算法对比（含全局优化） |
| ✅ **数值验证引擎** | 量纲分析、边界条件检查、交叉验证、敏感性分析、结果质量自动评估 |
| 🪤 **陷阱检测系统** | 数据异常检测、隐式约束识别、单位陷阱、数值陷阱、约束处理检查 |
| 🔄 **多算法对比** | 自动检测 ≥3 种算法实现（含全局优化+局部搜索），对比不同方案的结果 |
| 📊 **国赛级质量检测** | 结果质量比（vs 理论上界）、多目标覆盖率、蒙特卡洛鲁棒性自动评估 |
| 📝 **论文自动撰写** | 基于真实代码结果生成论文，支持 Claim-Evidence 映射 |
| 🛡️ **五道质量门禁** | M1/P1/P2/W1/W2 独立质检，每道最多 3 次重试，P0级问题自动 FAIL |
...
| 类别 | 变更内容 |
|------|---------|
| 🏆 **国赛模式** | `config.py` 启用国赛模式，`min_algorithm_count` 2→3，新增 `result_quality_threshold`(0.10)、`monte_carlo_success_rate`(0.60)、`coverage_threshold`(0.80) 三个质量阈值 |
| 📊 **质量自动检测** | `graph.py` 新增三大国赛级检测函数：`_check_result_quality_ratio`（结果质量 vs 理论上界）、`_check_multi_target_coverage`（多目标全覆盖）、`_check_monte_carlo_robustness`（蒙特卡洛鲁棒性），P0 级问题自动 FAIL |
| 🔢 **算法要求升级** | `coding.py` 算法对比从 2 种→**3 种**（必须包含 ≥1 种全局优化 + ≥1 种局部搜索），新增"国赛级结果质量要求"和"多目标全覆盖策略"章节 |
| ✅ **质检标准升级** | `quality.py` P2 算法完整性检查从 8 项→**11 项**（新增全局+局部算法、多目标全覆盖、蒙特卡洛鲁棒性、结果质量达标），不足 3 种算法直接判 FAIL |
| 📐 **理论上界要求** | `modeling.py` 新增"理论上界计算要求"章节，强制建模手在报告中给出具体数值的理论上界，用于后续自动评估优化结果质量 |
| 🐛 **Bug 修复** | 修复 `_check_negative_results` 对带单位数值的误检测（如 `-3.60s`），修复敏感性和文件路径关键词的误匹配，修复 `error_analysis_node` 对部分负值的误阻断 |
