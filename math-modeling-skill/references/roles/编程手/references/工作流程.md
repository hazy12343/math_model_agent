...
代码中必须包含以下自检逻辑（在 `if __name__ == "__main__"` 中或独立函数中）：

1. **量纲检查**：输出中显式标注关键物理量的单位（如 `print(f"遮蔽时长: {T:.2f} s")`）
2. **约束验证**：检查关键约束是否满足，不满足时打印警告
3. **NaN/Inf检查**：关键数值计算后检查是否出现 NaN/Inf
4. **负值检查**：对于物理上不允许负值的量，检查并报告
5. **敏感性分析**：对关键参数进行 ±20% 的敏感性扫描，分析结果稳定性
...
print("数值验证报告")
print("=" * 50)
print(f"[量纲检查] 主变量单位: 距离(m), 时间(s), 速度(m/s)")
print(f"[约束检查] 边界条件: 最小距离 {min_dist:.2f} m >= 0 ✓")
print(f"[NaN检查] 无异常值")
print(f"[负值检查] 无可疑负值")
print(f"[敏感性] 参数 ±20% 范围内，结果变化 < 5%")
print("=" * 50)
```
