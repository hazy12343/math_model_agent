...
# 计算效率规范（关键！防止代码超时）
- **禁止超过 3 层嵌套循环**：参数扫描类任务必须使用 NumPy 向量化或 itertools.product 替代嵌套 for 循环
  - 错误示例：`for theta in ...: for speed in ...: for t_drop in ...: for delay in ...:`  # 4 层嵌套 → 必定超时
  - 正确示例：使用 `np.meshgrid()` 向量化计算，或使用 `scipy.optimize.brute` 的向量化版本
- **时间步长不得小于 0.2s**：仿真类任务中，时间步长 Δt 必须 ≥ 0.2s，优先使用 0.5s
  - 对于 70s 时间窗口：Δt=0.1s → 700 步，Δt=0.5s → 140 步（5 倍加速）
- **差分进化参数限制**：`popsize` 不超过 10，`maxiter` 不超过 30，`tol` 设为 0.01 以提前终止
...
1. 计算搜索空间的特征尺度（如：无人机飞行范围约 20000m，方向角范围 2π rad）
2. 计算有效精度要求（如：有效遮蔽半径 10m，对应角度精度 = 10m / 20000m ≈ 0.0005 rad）
3. 计算所需最小搜索点数 = 搜索范围 / 有效精度 × 安全系数(≥3~5)
4. 例如：2π rad / 0.0005 rad × 5 ≈ 62800 个点 → 至少需要 60000 个角度采样点
5. 如果单层搜索点数过大（>10000），必须采用分层策略：
   - 第一层：粗搜索（500~1000 个点），找到最优解的大致区域
   - 第二层：在最优解附近 ±10% 范围内细搜索（2000~5000 个点），精化结果
   - 第三层：对最优解进行蒙特卡洛验证
6. 每层搜索必须输出当前最优结果，便于分析搜索收敛性
7. **关键原则：搜索分辨率必须由"有效精度"决定，不能随意设置！**
...
def fitness(params):
    ...
    # 示例：速度约束处理
    if v < V_MIN:
        penalty += 1000.0 * (V_MIN - v)  # 大惩罚系数
    if v > V_MAX:
        penalty += 1000.0 * (v - V_MAX)
    # 示例：时间约束处理
    if t_drop < 0:
        penalty += 10000.0 * abs(t_drop)
...
if constraint_violated:
    return 0.0  # 或 return -inf
```

# 资源充分利用（国赛关键）
- 必须使用全部可用资源（如：所有无人机、所有弹容量、所有时间窗口）
- 如果建模报告指定了资源上限，必须用满或给出未用满的合理理由
- 多弹协同策略：同一平台的多枚弹应形成连续或重叠的遮蔽时间窗口
- 输出中必须包含资源利用率统计
...
请完成：
1. 全量计算和参数扫描（敏感性分析）
2. 生成三类图（原始数据图、过程图、结果图），每类至少{self.config.min_figure_count // 3}张，合计至少{self.config.min_figure_count}张
3. 每个子问题在三类图中各至少1张
4. 生成结果表格
5. 生成复现清单
...
- **粗搜索总组合数 ≤ 3000**：如 8 方向角 × 4 速度 × 8 投放时刻 × 4 延迟 = 1024 组合（良好）
- 在代码中输出进度信息，如 `print(f"粗搜索进度: {{i+1}}/{{n_uav}}")`
...
### 资源充分利用（国赛关键）
- 必须使用全部可用资源（如：5架无人机全部使用，每架使用全部3枚弹容量）
- 如果资源未用满，必须在输出中说明原因
- 多弹协同策略：同一平台的多枚弹应形成连续或重叠的遮蔽时间窗口
