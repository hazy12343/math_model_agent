...
- 在建模报告中必须包含"搜索空间分析"章节，明确给出：
  1. 各搜索参数的范围和物理含义（如：θ ∈ [0, 2π)、v ∈ [70, 140] m/s）
...
- 理论上界计算方法：
  1. 识别所有并行资源（如多枚导弹、多条生产线、多个通道）
  2. 计算每个资源在其作用时间窗口内的最大贡献
  3. 理论上界 = 所有资源最大贡献之和（忽略资源冲突）
  4. 如果存在资源冲突，还应给出"考虑冲突的理论上界"
- 理论上界必须在报告中有清晰的计算步骤，格式如下：
  ```
  理论上界计算：
  - 资源A: 作用时间窗口 [t_start, t_end] = [0, 200]s，最大贡献 = 200s × 1 = 200s
  - 资源B: 作用时间窗口 [0, 200]s，最大贡献 = 200s × 1 = 200s
  - 理论上界（忽略冲突）= 200 + 200 = 400s
  - 考虑冲突的理论上界 = XXX（根据冲突约束计算）
  ```
