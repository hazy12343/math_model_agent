...
# 理论上界计算要求（国赛关键 — 用于评估优化结果质量）
- 对每个优化问题，必须在报告中明确给出理论上界值
- 理论上界计算方法：
  1. 识别所有并行资源（如多个执行单元、多条生产线、多个时间窗口）
  2. 计算每个资源在其有效作用范围内的最大贡献
  3. 理论上界 = 所有资源最大贡献之和（忽略资源间冲突）
  4. 如果存在资源冲突，还应给出"考虑冲突的理论上界"
- 理论上界必须在报告中有清晰的计算步骤，格式如下：
  ```
  理论上界计算：
  - 资源A: 有效作用范围 [t_start, t_end] = [0, T_max]，最大贡献 = T_max × K_A = X_A
  - 资源B: 有效作用范围 [0, T_max]，最大贡献 = T_max × K_B = X_B
  - 理论上界（忽略冲突）= X_A + X_B = X_total
  - 考虑冲突的理论上界 = X_conflict（根据冲突约束计算）
  ```
  其中 K_A、K_B 是题目给定的资源效率参数，T_max 是时间窗口长度
- 这个值将用于后续评估优化结果的质量（实际值/理论上界 应 ≥ 10%）
""")
        return "\n".join(parts)
