...
6. 对于M1建模终检：题目给出的物理定律和已知参数本身就是"依据"，无需额外引用外部文献来证明牛顿定律或运动学公式的正确性"""
...
6. 模型是否可实现（运动学模型+几何判定是非常标准的数学建模方法）
...
- 题目本身给出的物理定律（牛顿力学、运动学、几何判定）和数学方法无需引用外部文献
...
3. 图表是否覆盖了所需类型（轨迹图、过程图、结果图至少各1张）
...
9. 是否至少 2 种算法给出了有效对比结果（如果题目要求多算法求解）
10. 迭代算法是否检查了收敛性（输出"已收敛"或"未收敛"）
11. 是否进行了蒙特卡洛验证（±5% 扰动，≥100 次模拟，输出置信区间）
12. 是否进行了敏感性分析（至少 3 个参数，每个参数至少 5 个水平）
13. 最优解是否在可行域内（检查所有约束条件）

### 约束处理检查（国赛关键）
14. 如果使用了遗传算法/粒子群等迭代优化算法，是否正确处理了约束
    - 如果结果中出现 0.00 或全部为 0，说明约束处理可能存在问题
    - 正确的约束处理：惩罚函数法 / 可行解初始化 / 修复法
    - 错误做法：直接返回 0 作为不可行解的适应度值

中文显示检查：
- 检查代码中图表标签（set_title、set_xlabel、set_ylabel、label参数）是否使用中文
- 如果图表标签全部是英文，标记为 FAIL 并说明需要改为中文
- 中文标签示例：ax.set_title('M1 三维轨迹') ✓ 而非 ax.set_title('3D Trajectory') ✗

重要判定标准：
- 重点检查代码能否独立复现运行，结果是否在合理范围内
- 如果代码能执行、结果数值合理、图表覆盖了主要分析维度、图表标签为中文，就应返回PASS
- 不要因为图表美化程度、代码注释风格、或非关键参数的微小差异而判FAIL
- 如果代码存在语法错误无法执行、或结果数值明显违背物理规律（如遮蔽时长>20s）、或图表标签全部为英文、或数值验证发现P0级错误，才返回FAIL
- 注意：如果结果中包含"数值验证"部分，请认真审查其中的P0/P1级发现
- 对于算法完整性检查：如果代码中只包含一种算法，且题目需要优化求解，应标记为P1警告但不判FAIL
...
- 如果论文存在实质性的数值错误、主张与证据严重矛盾、或关键图表缺失，才返回FAIL
