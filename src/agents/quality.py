...
class QualityCheckAgent(BaseAgent):
    ...
    def load_system_prompt(self) -> str:
        ...
        # 加载各角色的质检清单
        m_qc = self._read_file(skill_root / "references/roles/建模手/references/质检清单.md")
        c_qc = self._read_file(skill_root / "references/roles/编程手/references/质检清单.md")
...
### 算法完整性检查（根据问题类型自适应）
首先识别建模报告中标注的问题类型（A=优化 / B=预测 / C=评价 / D=分类 / E=仿真 / F=机理 / G=统计），然后按对应类型检查：

**A. 优化类**：
8. 所有算法是否都返回了合理数值（非 0，非 NaN，非全部相同）
9. 是否至少 3 种算法给出了有效对比结果
10. 3 种算法中是否包含至少 1 种全局优化算法（差分进化/PSO/遗传算法/模拟退火）和至少 1 种局部搜索算法（贪心/爬山/坐标下降）
11. 迭代算法是否检查了收敛性（输出"已收敛"或"未收敛"）
12. 是否进行了蒙特卡洛验证（±5% 扰动，≥100 次模拟，输出置信区间）
13. 是否进行了敏感性分析（至少 3 个参数，每个参数至少 5 个水平）
14. 最优解是否在可行域内（检查所有约束条件）
15. 多目标问题中每个目标是否都有独立的非零结果
16. 蒙特卡洛均值是否 ≥ 最优值的 50%（注意：如果均值远低于最优值，检查扰动对象是否正确 — 应扰动模型参数而非控制参数）
17. 蒙特卡洛失败率（零值占比）是否 ≤ 20%
18. 优化结果是否 ≥ 理论最大值的 15%
19. **资源利用率（新增）**：所有可用资源节点是否都被使用？是否存在闲置资源（如 "FY1: 0/3"）？
20. **差分进化使用（新增）**：3 种算法中是否包含差分进化（differential_evolution）？如果只有贪心/随机搜索等简单算法，标记为 P1 警告

**B. 预测类**：
8. 是否划分了训练集/测试集并报告了划分比例
9. 是否至少 2-3 种预测模型给出了对比结果
10. 是否报告了预测误差指标（MAE/RMSE/MAPE/R² 至少 3 项）
11. 是否给出了预测的置信区间/预测区间
12. 是否进行了残差分析（残差图/QQ图/自相关图）
13. 是否分析了预测的不确定性来源

**C. 评价类**：
8. 是否至少 2 种评价方法给出了对比结果
9. 是否进行了权重敏感性分析（权重变化对排序的影响）
10. 是否检查了指标间的相关性（避免冗余）
11. 评价结果是否具有区分度（非所有方案得分相同）

**D. 分类类**：
8. 是否至少 2-3 种分类器给出了对比结果
9. 是否进行了 K 折交叉验证（K≥5）
10. 是否报告了混淆矩阵和 F1 值
11. 是否处理了类别不平衡问题
...
### 高维优化问题诊断（⚠️ 常见失败根因）
15. 如果结果全零或接近零，且代码中一次性优化了 >20 个参数，诊断原因为"维度爆炸"
    - 例如：bounds 中定义了 30-50 个参数维度的边界，DE 的 popsize=10, maxiter=30 只有 300 次评估
    - 300 次评估在 40 维空间中几乎等于随机采样，DE 几乎不可能找到可行解
    - 如果检测到此类问题，在返工建议中明确指出"参数维度≥X，建议使用分解优化（逐实体/逐维度优化）或随机采样+局部优化策略"
    - 如果代码中使用了 `for entity in range(N):` 逐实体优化而不是一次性优化所有参数，则此项检查通过
...
- **对于优化类问题，如果 3 种算法中不包含差分进化（differential_evolution）或等效全局优化算法，标记为 P1 警告**
- 对于约束处理检查：如果迭代算法返回0.00，应标记为P0错误并判FAIL
- **对于资源利用率检查：如果存在完全闲置的资源节点（如 "FY1: 0/3"），标记为 P0 错误并判 FAIL**
- **对于蒙特卡洛鲁棒性：如果 MC 均值 < 最优值的 30%，标记为 P0 错误并判 FAIL**
